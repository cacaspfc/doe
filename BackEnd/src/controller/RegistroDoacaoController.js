...
module.exports = {
  async store(req, res) {
    ...
    if (user) {
      if (antesDoacao) {
        lastRegister = await (await RegistroDoacao.find({'user': user_id}).sort({'dataDoacao': -1}));
      ...
      if (lastRegister.length !== 0) {
        var oldDonation = lastRegister[lastRegister.length - 1].proximaDataDoacao
  ...
  var proximaDataDoacao = new Date(moment(dataDoacao).subtract(1,"day"))
    if (user.genero == "Masculino"){
          ...
          registroDoacao.save();
            ...
            user: user._id,
            noTropy: tropy,
            proximaDataDoacao: new Date(proximaDataDoacao).setMonth(new Date(proximaDataDoacao).getMonth() + 4)
          });
          registroDoacao.save()
        }
  }
